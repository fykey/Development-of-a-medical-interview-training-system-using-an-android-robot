...
MAFfil_fanglex = filter(coeff15flame, 1, fanglex);
MAFfil_fangley = filter(coeff15flame, 1, fangley);


MAFfil_fanglex = filter(coeff15flame, 1, MAFfil_fanglex);
MAFfil_fangley = filter(coeff15flame, 1, MAFfil_fangley);
MAFfil_fanglex = filter(coeff15flame, 1, MAFfil_fanglex);
MAFfil_fangley = filter(coeff15flame, 1, MAFfil_fangley);
...
fil_time = time_stamps - 5*((fpsper2 - 1) / 2 / (fpsper2 * 2));

%{
HMPfil_fanglex = hampel(MAFfil_fanglex, fpsper2*6);
HMPfil_fangley = hampel(MAFfil_fangley, fpsper2*6);


plot(time_stamps,fangley ,fil_time,MAFfil_fangley, fil_time, HMPfil_fangley)
legend('before','MAF filter', 'HMP filter','location','best')
ylabel('angle (deg)')
xlabel('Time (sec)')
...
%角速度算出
for i = 1:(renum)
    fvangleX(i,1) = (MAFfil_fanglex(i+1,1) - MAFfil_fanglex(i,1)) / (fil_time(i+1,1) - fil_time(i,1));
    fvangleY(i,1) = (MAFfil_fangley(i+1,1) - MAFfil_fangley(i,1)) / (fil_time(i+1,1) - fil_time(i,1));
end
...
folder_name = strcat('noise_filter_',string(noise_param), '_', name);
mkdir(folder_name);
